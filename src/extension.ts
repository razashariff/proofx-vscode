...
function getConfig() {
  ...
  return {
    ...
    creatorId: config.get<string>('creatorId') || '',
    autoSign: config.get<boolean>('autoSign') || false,
  };
}
...
function apiRequest(endpoint: string, body: object, apiKey: string): Promise<any> {
...
async function signFile(uri: vscode.Uri) {
  ...
  if (!apiKey || !creatorId) {
    const setup = await vscode.window.showErrorMessage(
      'ProofX: API Key and Creator ID required. Get them free at proofx.co.uk/developer',
      'Set API Key', 'Set Creator ID'
    );
  ...
  try {
    ...
    if (result.content_id) {
      ...
      const action = await vscode.window.showInformationMessage(
        `ProofX: ${fileName} signed successfully!`,
        'Copy Verification URL', 'Open in Browser'
      );
...
async function verifyFile(uri: vscode.Uri) {
  ...
  if (currentHash === proof.hash) {
    const action = await vscode.window.showInformationMessage(
      `ProofX: ${fileName} is VERIFIED. Untampered since ${proof.signed_at}`,
    ...
    vscode.window.showWarningMessage(
      `ProofX: ${fileName} has been MODIFIED since signing!\n` +
...
async function signFolder(uri: vscode.Uri) {
  const { apiKey, creatorId } = getConfig();
  ...
  const files = getAllFiles(folderPath).filter(f =>
    !f.includes('.proofx') && !f.includes('node_modules') && !f.includes('.git')
  );
  ...
  const confirm = await vscode.window.showInformationMessage(
    `ProofX: Sign ${files.length} files in ${path.basename(folderPath)}?`,
    'Sign All', 'Cancel'
  );

  if (confirm !== 'Sign All') { return; }
...
export function activate(context: vscode.ExtensionContext) {
  context.subscriptions.push(
    vscode.commands.registerCommand('proofx.signFile', signFile),
    vscode.commands.registerCommand('proofx.verifyFile', verifyFile),
    vscode.commands.registerCommand('proofx.signFolder', signFolder),

    vscode.commands.registerCommand('proofx.setApiKey', async () => {
